<?php
// admin_estoque.php (ou estoque.php)
if (!isset($_SESSION['user_id'])) exit;

// Apenas Admin (Opcional, conforme sua regra)
// if ($_SESSION['user_role'] != 'admin') exit('Acesso Negado');

// --- LÓGICA DE SINCRONIZAÇÃO COM TRACCAR ---
function syncTraccar($pdo, $imei, $model, $existingTcId = null) {
    if (empty($imei)) return null;
    $deviceName = "$model - $imei";

    // 1. Verifica se já existe esse IMEI no Traccar
    $stmt = $pdo->prepare("SELECT id FROM tc_devices WHERE uniqueid = ?");
    $stmt->execute([$imei]);
    $foundId = $stmt->fetchColumn();

    if ($foundId) {
        // Se já existe, atualiza o nome
        $pdo->prepare("UPDATE tc_devices SET name = ? WHERE id = ?")->execute([$deviceName, $foundId]);
        return $foundId;
    } else {
        // Se não existe, cria novo
        $stmtIns = $pdo->prepare("INSERT INTO tc_devices (name, uniqueid) VALUES (?, ?) RETURNING id");
        $stmtIns->execute([$deviceName, $imei]);
        return $stmtIns->fetchColumn();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $action = $_POST['action'];
        $tenant_id = $_SESSION['tenant_id'] ?? $tenant['id']; // Garante tenant correto
        ...
        if ($action == 'create') {
            ...
            // Se for rastreador, cria no Traccar
            $traccarId = null;
            if ($type == 'tracker') {
                $traccarId = syncTraccar($pdo, $identifier, $model);
            }
            ...
            $pdo->commit();
            echo "<script>showToast('Item criado e sincronizado com Traccar!', 'green');</script>";
        }
        ...
        if ($action == 'update') {
            ...
            $traccarId = null;
            if ($type == 'tracker') {
                // Busca ID atual se houver
                $stmtGet = $pdo->prepare("SELECT traccar_device_id FROM saas_stock WHERE id = ?");
                $stmtGet->execute([$id]);
                $currentTcId = $stmtGet->fetchColumn();
                
                $traccarId = syncTraccar($pdo, $identifier, $model, $currentTcId);
            }
            ...
            $pdo->commit();
            echo "<script>showToast('Item atualizado com sucesso!', 'blue');</script>";
        }
        ...
        if ($action == 'delete') {
            ...
            // 1. Pega o ID do Traccar antes de excluir
            ...
            // 3. Exclui do Traccar (Se existir)
            if ($tcId) {
                // Opcional: Verificar se tem histórico antes de excluir, mas como pediu para excluir...
                $pdo->prepare("DELETE FROM tc_devices WHERE id = ?")->execute([$tcId]);
            }

            $pdo->commit();
            echo "<script>showToast('Item e dispositivo removidos.', 'red');</script>";
        }
                ...
                <tbody class="divide-y divide-gray-100 text-sm text-slate-600" id="stock-list">
                    ...
                    <tr class="hover:bg-slate-50 transition group row-item" data-type="<?php echo $i['type']; ?>" data-search="<?php echo strtolower($i['identifier'].' '.$i['model'].' '.$i['brand']); ?>">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            <form method="POST" class="inline" onsubmit="return confirm('Remover item? Se for rastreador, será excluido do Traccar também.');">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <form method="POST" class="p-6">
            ...
            <div class="mb-4">
                ...
                <p class="text-[10px] text-gray-400 mt-1">Se for rastreador, este IMEI será criado na plataforma.</p>
            </div>
